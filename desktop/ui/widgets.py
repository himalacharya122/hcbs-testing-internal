...
from desktop.ui.theme import (
    ACCENT, DANGER, SUCCESS, SMOKE, SILVER, SNOW, WHITE, CHARCOAL, BLACK,
...
def status_badge(status: str) -> QLabel:
    """Booking status badge — confirmed (green) or cancelled (red)."""
...
# Cards
class Card(QFrame):
    """A bordered card container with rounded corners."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFrameShape(QFrame.Shape.StyledPanel)
        self.setStyleSheet(
            f"Card {{ background: {WHITE}; border: 1px solid {SILVER}; "
...
def form_row(label_text: str, widget: QWidget, stretch: bool = True) -> QHBoxLayout:
    ...
    lbl.setStyleSheet(f"color: {CHARCOAL}; font-weight: 500;")
    row.addWidget(lbl)
    if stretch:
...
def labelled_value(label: str, value: str) -> QHBoxLayout:
    ...
    lbl = QLabel(label)
    lbl.setFont(body_font(10))
    lbl.setStyleSheet(f"color: {SMOKE}; font-weight: 500;")
    lbl.setFixedWidth(130)
    val = QLabel(value)
    val.setFont(body_font(11))
    val.setStyleSheet(f"color: {CHARCOAL};")
...
def separator() -> QFrame:
    ...
    line.setStyleSheet(f"background-color: {SILVER}; max-height: 1px;")
    return line
...
class Toast(QLabel):
    ...
    def __init__(self, parent, message: str, colour: str = CHARCOAL, duration: int = 3000):
